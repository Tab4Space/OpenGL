...
bool Context::Init()
{
    ...
    */
    ...
    // ssao 커널을 위한 16개(4x4 texture) 랜덤 rotate vector 초기화
    std::vector<glm::vec3> ssaoNoise;
    ssaoNoise.resize(16);
    for (size_t i = 0; i < ssaoNoise.size(); i++) 
    {
        // 랜덤한 tangent 생성, normal 방향이 z축에 거의 평행할 것이기 때문에 z축은 0으로두고 x, y축만 -1 ~ 1 랜덤 세팅
    ...
    // 4x4 텍스처를 만들고 이미지 전체에 타일링
    m_ssaoNoiseTexture->SetWrap(GL_REPEAT, GL_REPEAT);
    // 텍스처를 위한 메모리 공간은 확보되어 있지만 비어있는 것에 값을 채워 넣는다
    ...
    for (size_t i = 0; i < m_ssaoSamples.size(); i++) 
    {
        ...
        glm::vec3 sample(RandomRange(-1.0f, 1.0f), RandomRange(-1.0f, 1.0f), RandomRange(0.0f, 1.0f));
        // normalize 하면 xy plane 위쪽에 반구 안의 어떤 점이다
        sample = glm::normalize(sample) * RandomRange();

        // 원점에 조금 더 가깝게 샘플의 위치를 조정한다
...
void Context::Render()
{
    ...
    // deffered shading second pass rendering
    // first pass에서 나온 3개의 텍스처 바인딩
