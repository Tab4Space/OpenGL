...
static GLenum GetImageFormat(uint32_t internalFormat) 
{
    ...
    else if (internalFormat == GL_RED || internalFormat == GL_R || internalFormat == GL_R16F || internalFormat == GL_R32F) 
    {
        imageFormat = GL_RED;
    }
    return imageFormat;
...
bool CubeTexture::InitFromImages(const std::vector<Image*>& images)
{
    ...
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);      // x축
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);      // y축
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);      // z축
